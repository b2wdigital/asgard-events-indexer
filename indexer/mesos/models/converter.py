...
class MesosEventConverter(ModelConverter[Event, MesosEvent]):
    @staticmethod
    def to_client_model(other: Event) -> MesosEvent:
        raise NotImplementedError

    @staticmethod
    def to_asgard_model(other: MesosEvent) -> Event:
        if other.type == "TASK_ADDED":
            task_id = other.task_added.task.task_id.value
            agent_id = other.task_added.task.agent_id.value
            return Event(
                id=str(uuid4()),
                ...
                status=other.task_added.task.state,
            )
        return f"UNSUPORTED_EVENT: {other.type}"
